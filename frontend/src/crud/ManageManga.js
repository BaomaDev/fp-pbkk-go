...
const ManageManga = () => {
    ...
    const fetchMangas = async () => {
        try {
            const response = await axios.get("http://localhost:8080/manga");
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editingManga) {
                // Update manga
                await axios.put(`http://localhost:8080/manga/${editingManga.id}`, formData);
                alert("Manga updated successfully!");
            } else {
                // Create new manga
                await axios.post("http://localhost:8080/manga", formData);
    ...
    const handleDelete = async (id) => {
        if (window.confirm("Are you sure you want to delete this manga?")) {
            try {
                await axios.delete(`http://localhost:8080/manga/${id}`);
